/*
 * CSN.1 definitions from 3GPP TS 44.060.
 *
...
static gint16 Egprs_Ack_Nack_Desc_w_len_Dissector(csnStream_t* ar, struct bitvec *vector, unsigned *readIndex, void* data)
{
  ...
  {
    return osmo_csn1_stream_decode(ar, CSNDESCR(EGPRS_AckNack_Desc_t), vector, readIndex, data);
  }
}
...
gint16 Receive_N_PDU_Number_list_Dissector(csnStream_t* ar, struct bitvec *vector, unsigned *readIndex, void* data)
{
  ...
  {
    return osmo_csn1_stream_decode(ar, CSNDESCR(Receive_N_PDU_Number_t), vector, readIndex, data);
  }
}
#endif
...
int osmo_gprs_rlcmac_decode_uplink(struct bitvec *vector, RlcMacUplink_t * data)
{
  ...
  data->NrOfBits = 23 * 8;
  osmo_csn1_stream_init(&ar, 0, data->NrOfBits);
  readIndex += 6;
...
int osmo_gprs_rlcmac_decode_egprs_pkt_ch_req(guint16 ra, EGPRS_PacketChannelRequest_t *req)
{
  ...
  osmo_csn1_stream_init(&ar, 0, 11);

  /* Recursive osmo_csn1_stream_decode call uses LOGPC everywhere, so we need to start the log somewhere... */
  LOGP(DLCSN1, LOGL_DEBUG, "osmo_csn1_stream_decode (EGPRS Packet Channel Request): ");
  ret = osmo_csn1_stream_decode(&ar, CSNDESCR(EGPRS_PacketChannelRequest_t),
				&bv, &readIndex, (void *) req);
  LOGPC(DLCSN1, LOGL_DEBUG, "\n");
